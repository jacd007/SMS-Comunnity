...
public class ExcelUtil {
    ...
    public static List<ContactsModel> readExcelNew(Context context, Uri uri, String filePath) {
        String nameFile = Utils.getToday("yyyyMMdd-HHmmss");
        try{
            String path = filePath.split(":")[1];
            nameFile = ""+path.split("/")[path.split("/").length-1];
        ...
        try {
            ...
            if (wb != null) {
                ...
                Log.i(TAG, "---- readExcelNew ----");
                for (int i = 1; i < rownum; i++) {
                    Row row = sheet.getRow(i);
                    //storing subcontent
                    Map<Integer, Object> itemMap = new HashMap<>();
                    if (row != null) {
                        String storyTitle = getCellFormatValue(row.getCell(0)).toString();
                        String storyContent = getCellFormatValue(row.getCell(1)).toString();
                        Log.i(TAG, "name: " + storyTitle+" phone: "+storyContent);
    ...
    private static void goExcel(Uri uri) {
        try{
